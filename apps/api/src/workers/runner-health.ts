import { finalizeRunIfComplete } from '../routes/runners';
...
export async function checkRunnerHealth() {
  ...
  try {
    const runIds = await db.transaction(async (tx) => {
      // Mark runners as offline if they haven't sent a heartbeat recently
      const offlineRunners = await tx
        .update(runners)
      ...
      if (offlineRunners.length > 0) {
        console.log(
          `[RunnerHealth] Marked ${offlineRunners.length} runner(s) offline:`,
          offlineRunners.map((r) => r.name),
        );
      }
      ...
      if (offlineRunnerIds.length > 0) {
        ...
        if (stalledJobs.length > 0) {
          console.log(
            `[RunnerHealth] Failed ${stalledJobs.length} stalled job(s) from offline runners`,
          );
        ...
        return [...new Set(stalledJobs.map((job) => job.runId))];
      }
      return [];
    });
    for (const runId of runIds) await finalizeRunIfComplete(runId, now);
  } catch (err) {
